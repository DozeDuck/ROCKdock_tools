...
/usr/bin/obabel -isdf $1 -O separated.sdf -m #可以把seesar_rescore_fullposes.sdf用$1表示
awk '/LOWER_BOUNDARY/{getline a;print a}' $1 > hyde_LOWER_score
...
for i in $abc
do
	cat separated$i.sdf >> seesar_rescore_subset_top_$2.sdf
done
